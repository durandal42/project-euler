...
def euler028(size):
  ...
  for count in itertools.count(1):
    ...
    if data[i+ndi][j+ndj] is None:  # if there's nothing one step in our new direction...
      di, dj = ndi, ndj  # ... commit to it.

  return sum(data[i][i] + data[i][size-i-1] for i in range(size)) - 1  # don't double-count center
